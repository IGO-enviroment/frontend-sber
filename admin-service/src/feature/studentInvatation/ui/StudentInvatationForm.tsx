...
import {
  StudentInvatationFormSchema,
  studentInvatationFormSchema,
} from "../model/types/studentInvatationFormSchema";
  ...
  return (
    <Stack spacing={2} sx={{ display: "flex", justifyContent: "center" }} p={2}>
          ...
          <Stack spacing={2}>
            ...
            {userFields.map((field, index) => (
              ...
              >
                <TextField
                  label="Имя"
                  {...register(`students.${index}.firstName` as const)}
                />
                <TextField
                  label="Почта"
                ...
                <TextField
                  label="Фамилия"
                  {...register(`students.${index}.lastName` as const)}
                />
                <TextField
                  label="Отчество"
                  {...register(`students.${index}.middleName` as const)}
                />
                <TextField
                  label="Телефон"
                  {...register(`students.${index}.phoneNumber` as const)}
                />
      ...
      <Stack direction="row" justifyContent="center" spacing={2}>
        <LoadingButton
          loading={isLoading}
          onClick={handleSubmit(onCreationSubmit)}
          variant="contained"
        >
